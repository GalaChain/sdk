...
import { genKey, generateKeyFromClassKey, validateTokenOrder } from "./helpers/format.helper";
import { sqrtPriceToTick } from "./helpers/tick.helper";
import { Pool } from "./pool";
import { GetUserPositionResponse, PositionsObject } from "./types";
 ...
 * @returns
 */
export async function getPoolData(ctx: GalaChainContext, dto: GetPoolDto): Promise<Pool> {
...
export async function positions(ctx: GalaChainContext, dto: GetPositionDto): Promise<positionInfoDto> {
  const pool = await getPoolData(ctx, dto);
  let key = genKey(dto.owner, dto.tickLower.toString(), dto.tickUpper.toString());
  // let key = `${dto.owner}_${dto.tickLower}_${dto.tickUpper}`;
