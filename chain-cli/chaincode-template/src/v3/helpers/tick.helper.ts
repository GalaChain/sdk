...
import { Default } from "fabric-contract-api";
...
export function flipTick(bitmap: Map<number, string>, tick: number, tickSpacing: number) {
  if (tick % tickSpacing != 0) {
    throw new Error("Tick is not spaced " + tick + " " + tickSpacing);
  }
  tick /= tickSpacing;
  let word = tick >> 8;
  let pos = Math.abs(tick % 256);
...
export function nextInitialisedTickWithInSameWord(
  ...
  if (lte) {
    ...
    let pos = Math.abs(compressed % 256);
    ...
    // return [((word << 8) + newPos) * tickSpacing,  intialized];
  } else {
...
//     const getBitmask = (word: number) => BigInt(bitmap[word] ?? "0");
//     const word = compressed >> 8;
//     const pos = Math.abs(compressed % 256);
...
//         const newPos = initialized ? mostSignificantBit(masked) : 0;
...
//         const initialized = masked !== BigInt(0);
//         const newPos = initialized ? leastSignificantBit(masked) : 255; // Default to last position if uninitialized
//         const value = initialized ? compressed + (newPos - pos) : compressed + (256 - pos);
//         return [value * tickSpacing, initialized];
//     }
// }
