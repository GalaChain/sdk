...
describe("nextInitialisedTickWithInSameWord (Object-based)", () => {
  test("should return the nearest initialized tick when lte=true", () => {
    ...
    expect(nextInitialisedTickWithInSameWord(bitmap as any, 2, 1, true)).toEqual([1, true]);
    expect(nextInitialisedTickWithInSameWord(bitmap as any, 4, 1, true)).toEqual([3, true]);
    expect(nextInitialisedTickWithInSameWord(bitmap as any, 0, 1, true)).toEqual([0, false]);
  });
  ...
  test("should return the nearest initialized tick when lte=false", () => {
    ...
    expect(nextInitialisedTickWithInSameWord(bitmap as any, 1, 1, false)).toEqual([3, true]);
    expect(nextInitialisedTickWithInSameWord(bitmap as any, 2, 1, false)).toEqual([3, true]);
    expect(nextInitialisedTickWithInSameWord(bitmap as any, 3, 1, false)).toEqual([255, false]);
  });
  ...
  test("should handle negative ticks", () => {
    ...
    expect(nextInitialisedTickWithInSameWord(bitmap as any, -3, 1, true)).toEqual([-3, true]);
    expect(nextInitialisedTickWithInSameWord(bitmap as any, -5, 1, true)).toEqual([-7, true]);
  });

  test("should handle empty bitmaps", () => {
    const bitmap: { [key: number]: string } = {};

    expect(nextInitialisedTickWithInSameWord(bitmap as any, 10, 1, true)).toEqual([0, false]);
    expect(nextInitialisedTickWithInSameWord(bitmap as any, 10, 1, false)).toEqual([255, false]);
  });
  ...
  test("should handle large tick spacings", () => {
    ...
    expect(nextInitialisedTickWithInSameWord(bitmap as any, 512, 256, true)).toEqual([512, true]);
    expect(nextInitialisedTickWithInSameWord(bitmap as any, 256, 256, false)).toEqual([512, true]);
  });
});
...
describe("tickCross function", () => {
  test("Initializes tick data if it does not exist", () => {
    ...
    const result = tickCross(tick, tickData as Map<number, TickData>, feeGrowthGlobal0, feeGrowthGlobal1);
