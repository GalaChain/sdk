...
describe("VestingToken", () => {
  test("Create Vesting Token", async () => {
    ...
    const vestingTokenDto: CreateVestingTokenDto = await createValidSubmitDTO(CreateVestingTokenDto, {
      tokenClass: currency.tokenClass,
      start: 123,
      vestingName: "SuperTokenTGE",
      //vestingTimeIncrement: "Days",
      allocations: [
        {
          name: "allocation1",
          owner: users.testUser1Id,
          quantity: new BigNumber(100),
          cliff: new BigNumber(90),
          vestingPeriod: new BigNumber(730)
        },
        {
          name: "allocation2",
          owner: users.testUser2Id,
          quantity: new BigNumber(50),
          cliff: new BigNumber(1),
          vestingPeriod: new BigNumber(730)
        },
        {
          name: "allocation3",
          ...
          cliff: new BigNumber(0),
          vestingPeriod: new BigNumber(0)
        }]
    });

    const expectedCreateVestingTokenResult = new CreateVestingTokenResult(
      {
    ...
    const { ctx, contract, writes } = fixture(GalaChainTokenContract)
      ...
      .savedState(currency.tokenClass()); //,balances?);

    // When
    const response = await contract.TransferToken(ctx, vestingTokenDto);
  ...
  //   const { ctx, contract, writes } = fixture(GalaChainTokenContract)
  //     .callingUser(users.attacker)
  //     .savedState(currencyClass, currencyInstance, transferAllowance, ownerBalance);
